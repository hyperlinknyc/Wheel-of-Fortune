 ...
 * total board height, shrinking the tile further (down to a 21px floor)
 * when a puzzle has many rows.
 *
 * The height budget is measured, not assumed. It used to be a flat share of
 * the screen, which cannot know what else this particular screen is carrying:
 * on the vowel drill -- category, board, pot line, and a three-row action bar
 * -- 42% was enough to push the pot, the one number the decision is about,
 * under the fold on an iPhone SE. Everything except the board is already laid
 * out by the time this runs, so ask the DOM what is left instead.
 */
export function sizeBoard(board) {
  const wrap = board.parentElement;
  const screen = screenEl();
  const avail = Math.min((wrap?.clientWidth || screen?.clientWidth || 340), 560);
  ...
  // Everything on the screen that is not this board, margins included:
  // scrollHeight counts the wrap's margins while offsetHeight does not, so
  // this errs a few pixels toward a smaller board, which is the safe way to
  // be wrong.
  const other = Math.max(0, (screen?.scrollHeight || 0) - (wrap?.offsetHeight || 0));
  // A board marked .compact is context beside something else (a letter grid,
  // a picker) rather than the main event, so it is capped tighter still.
  const cap = wrap?.classList.contains('compact') ? screenH * 0.30 : Infinity;
  const heightBudget = Math.max(90, Math.min(cap, screenH - other));
  const rowGap = 6;
